...
3.检查所有项目（J3DEngine、J3DGUI、J3DMVSEngine、Viewer）的项目属性，并检查各个三方库的include目录（C++-常规-附加包含目录）、lib目录（链接器-常规-附加库目录）（链接器-输入-附加依赖项）是否正确（我的三方库统一安装在C:\openMVS下，如果不是安装于此，需要进行相关的修改）

4.按照J3DMVSEngine、Viewer、J3DGUI、J3DEngine的顺序，生成各个项目。
